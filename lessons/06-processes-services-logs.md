# Lesson 6: Linux processes, services, systemd, logs, CPU, memory, and storage
...
## Learning objectives
After this lesson you will be able to explain processes, services, systemd, journal logs, resource checks, and disk safety, verify your environment, run safe commands, read command output, document findings in `botera.md`, and apply a structured troubleshooting process. You will also improve the continuous Botera learning website project used throughout the course.

## Prerequisites
- **Previous lessons required:** Lessons 1 through 5.
...
### `top`
- **Utility:** `top` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `systemctl status`
- **Utility:** `systemctl` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `journalctl -u`
- **Utility:** `journalctl` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `df -h`
- **Utility:** `df` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `du -sh`
- **Utility:** `du` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `free -h`
- **Utility:** `free` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
...
**Scenario:** A service fails after restart and logs reveal a missing file.
...
```bash
mkdir -p ~/botera-course/lesson-06
cd ~/botera-course/lesson-06
printf '# Lesson 6 notes\nStatus: started\n' > botera.md
cat botera.md
```
...
The continuous project is the **Botera DevOps and Network Engineering Course Website**. In this lesson, add or update `botera.md` with a short operational note for lesson 6. The website reads course Markdown from the repository, so documentation becomes part of the product. In real teams, this pattern becomes a runbook or internal developer portal.

## Guided practical lab
**Lab scenario:** You are onboarding to a company and must prove you can perform lesson 6 tasks safely.
...
**Directory structure:** `labs/lesson-06/README.md` plus your local `~/botera-course/lesson-06` workspace.
**Files to create:** `botera.md` and `evidence.txt`.
```bash
mkdir -p ~/botera-course/lesson-06
cd ~/botera-course/lesson-06
printf 'lesson=6\nstatus=lab-started\n' > evidence.txt
printf '# Botera lesson 6 lab\n\nEvidence recorded.\n' > botera.md
cat evidence.txt
cat botera.md
```
...
**Cleanup:** `cd ~ && rm -rf ~/botera-course/lesson-06` if you want to remove the lab.
...
**Scenario:** A teammate needs repeatable notes for lesson 6.
**Goal:** create `~/botera-course/exercise-06/botera.md` with a heading, date, three facts learned, and one open question.
...
**Verification criteria:** `test -s ~/botera-course/exercise-06/botera.md && echo complete`.

## Complete exercise solution
```bash
mkdir -p ~/botera-course/exercise-06
cd ~/botera-course/exercise-06
cat > botera.md <<'EOF'
# Lesson 6 independent exercise
...
## Homework
Research one term from this lesson, run each listed command twice, create `homework/lesson-06-notes.md`, intentionally cause and fix one safe error, and add one improvement idea for the Botera project. Completion criteria: your note contains command output, one troubleshooting story, and verification steps. Verify with `test -s homework/lesson-06-notes.md`.
...
## Lesson summary
You learned processes, services, systemd, journal logs, resource checks, and disk safety, practiced commands including `ps aux`, `top`, `systemctl status`, `journalctl -u`, `df -h`, created files, verified output, studied common errors, and connected beginner practice to production best practices.

## Connection to the next lesson
This lesson prepares you for **Networking foundations, OSI model, TCP/IP model, packets, frames, ports, and protocols** by giving you vocabulary, safe command habits, and documentation discipline that the next topic depends on.
