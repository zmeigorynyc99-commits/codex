# Lesson 9: Linux networking utilities and systematic network troubleshooting
...
## Learning objectives
After this lesson you will be able to explain ip, ping, ss, curl, dig/getent, traceroute, tcpdump basics, and troubleshooting method, verify your environment, run safe commands, read command output, document findings in `botera.md`, and apply a structured troubleshooting process. You will also improve the continuous Botera learning website project used throughout the course.

## Prerequisites
- **Previous lessons required:** Lessons 1 through 8.
...
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `ping -c 4`
- **Utility:** `ping` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
...
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `dig`
- **Utility:** `dig` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `traceroute`
- **Utility:** `traceroute` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
- **Common diagnoses:** wrong directory, missing package, stopped service, bad permission, unavailable network path, or incorrect Git state.
### `tcpdump`
- **Utility:** `tcpdump` is the program being executed.
- **Arguments/options:** the remaining words narrow what the utility should inspect or change.
- **Why used:** it provides observable evidence instead of assumptions.
- **Output:** names, paths, addresses, statuses, or errors may differ on your system.
- **Professional use:** engineers run this during verification, troubleshooting, documentation, or automation.
...
**Scenario:** DNS resolves but the TCP connection times out through a firewall.
...
```bash
mkdir -p ~/botera-course/lesson-09
cd ~/botera-course/lesson-09
printf '# Lesson 9 notes\nStatus: started\n' > botera.md
cat botera.md
```
...
The continuous project is the **Botera DevOps and Network Engineering Course Website**. In this lesson, add or update `botera.md` with a short operational note for lesson 9. The website reads course Markdown from the repository, so documentation becomes part of the product. In real teams, this pattern becomes a runbook or internal developer portal.

## Guided practical lab
**Lab scenario:** You are onboarding to a company and must prove you can perform lesson 9 tasks safely.
...
**Directory structure:** `labs/lesson-09/README.md` plus your local `~/botera-course/lesson-09` workspace.
**Files to create:** `botera.md` and `evidence.txt`.
```bash
mkdir -p ~/botera-course/lesson-09
cd ~/botera-course/lesson-09
printf 'lesson=9\nstatus=lab-started\n' > evidence.txt
printf '# Botera lesson 9 lab\n\nEvidence recorded.\n' > botera.md
cat evidence.txt
cat botera.md
```
...
**Cleanup:** `cd ~ && rm -rf ~/botera-course/lesson-09` if you want to remove the lab.
...
**Scenario:** A teammate needs repeatable notes for lesson 9.
**Goal:** create `~/botera-course/exercise-09/botera.md` with a heading, date, three facts learned, and one open question.
...
**Verification criteria:** `test -s ~/botera-course/exercise-09/botera.md && echo complete`.

## Complete exercise solution
```bash
mkdir -p ~/botera-course/exercise-09
cd ~/botera-course/exercise-09
cat > botera.md <<'EOF'
# Lesson 9 independent exercise
...
## Homework
Research one term from this lesson, run each listed command twice, create `homework/lesson-09-notes.md`, intentionally cause and fix one safe error, and add one improvement idea for the Botera project. Completion criteria: your note contains command output, one troubleshooting story, and verification steps. Verify with `test -s homework/lesson-09-notes.md`.
...
## Lesson summary
You learned ip, ping, ss, curl, dig/getent, traceroute, tcpdump basics, and troubleshooting method, practiced commands including `ip link`, `ip addr`, `ping -c 4`, `curl -v`, `ss -ltnp`, created files, verified output, studied common errors, and connected beginner practice to production best practices.

## Connection to the next lesson
This lesson prepares you for **Git, GitHub, branches, commits, pull requests, and version control for infrastructure** by giving you vocabulary, safe command habits, and documentation discipline that the next topic depends on.
